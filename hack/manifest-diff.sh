...
if [ "$NO_DOCKER" = "1" -o -n "$IS_CONTAINER" ]; then
  ...
  csvfile="manifests/4.11/vertical-pod-autoscaler.clusterserviceversion.yaml"
  ...
  crdfile="manifests/4.11/vertical-pod-autoscaler-controller.crd.yaml"
  ...
  crdfile="manifests/4.11/vpa-v1.crd.yaml"
  ...
  crdfile="manifests/4.11/vpacheckpoint-v1.crd.yaml"
